...
def generate_forklist(repo: str, branch: str, include_identical: bool):
   ...
   assert forks_result is not None
   ...
   def get_fork_details(
      fork_repo: str, fork_branch: str, upstream_repo: list[str], upstream_branch: str
   ):

      comp_endpoint = f"/repos/{fork_repo}/compare/{upstream_repo[0]}:{upstream_repo[1]}:{upstream_branch}...{fork_branch}"
   ...
   df_status = df.apply(
      lambda x: get_fork_details(
         fork_repo=x.full_name,
         fork_branch=x.default_branch,
         upstream_repo=repo_ls,
...
def build_parser() -> argparse.ArgumentParser:
   ...
   p.add_argument(
      "repo", metavar="REPO", help="The target Github repository in owner/repo format"
   )
